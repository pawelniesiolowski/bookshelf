...
class Book
{
    ...
    private function calculateEvents(array $events): void
    {
        foreach ($events as $event) {
            if ($event->name() === BookChangeEvent::RECEIVE) {
                $this->copies = $this->copies->add($event->copies());
            } elseif ($event->name() === BookChangeEvent::RELEASE || $event->name() === BookChangeEvent::SELL) {
                $this->copies = $this->copies->subtract($event->copies());
            }
        }
    }
}
